...
inline constexpr std::uint32_t HEX_PADDING       = 2;                                                                         // The distatnce between the line byte offset and the ASCII digits.
inline constexpr std::uint32_t LINE_OFFSET_LEN   = SUPPORT_BIG_FILES ? sizeof(std::uint64_t) * 2 : sizeof(std::uint32_t) * 2; // The number of hex digits used for the line byte offset.
